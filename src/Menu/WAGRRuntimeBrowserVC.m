...
extern BOOL WAGRWAABOriginalBoolForFlag(NSString *flag, BOOL *knownOut);
...
@property BOOL systemValueKnown;
@property BOOL systemValue;
@end
@implementation WAGRMethodEntry
@end

static id WAGRBRProbeObjectForClass(Class cls) {
    if (!cls) return nil;
    NSArray<NSString *> *candidates = @[@"sharedInstance", @"shared", @"defaultInstance", @"defaultProperties", @"sharedProperties", @"sharedABProperties", @"currentProperties", @"properties", @"instance"];
...
static BOOL WAGRBRReadSystemValue(NSString *className, NSString *methodName, BOOL isClassMethod, BOOL *knownOut) {
    if (knownOut) *knownOut = NO;
    if (!className.length || !methodName.length) return NO;

    if ([className containsString:@"WAABProperties"] || [className containsString:@"ABProperties"]) {
        BOOL known = NO;
        BOOL value = WAGRWAABOriginalBoolForFlag(methodName, &known);
        if (known) { if (knownOut) *knownOut = YES; return value; }
    }

    Class cls = NSClassFromString(className);
    SEL sel = NSSelectorFromString(methodName);
    if (!cls || !sel) return NO;
    id target = isClassMethod ? (id)cls : WAGRBRProbeObjectForClass(cls);
...
static BOOL WAGRBREffective(WAGRMethodEntry *e) {
    if ([e.overrideState isEqualToString:@"on"]) return YES;
    if ([e.overrideState isEqualToString:@"off"]) return NO;
    return e.systemValueKnown ? e.systemValue : NO;
}
...
- (void)startScan {
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        for (NSString *clsName in self.targetClassNames) {
            ...
            for (int meta = 0; meta <= 1; meta++) {
                ...
                for (unsigned int i = 0; i < n; i++) {
                    ...
                    e.overrideState = WAGRStoredStateForFlag(sel);
                    BOOL known = NO;
                    BOOL sys = WAGRBRReadSystemValue(clsName, sel, (BOOL)meta, &known);
                    e.systemValueKnown = known;
                    e.systemValue = sys;
                    [entries addObject:e];
                }
                free(ms);
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSUInteger on = 0;
            for (WAGRMethodEntry *e in entries) if (WAGRBREffective(e)) on++;
            self.statusLabel.text = [NSString stringWithFormat:@"%lu métodos (%lu efetivos ON)", (unsigned long)entries.count, (unsigned long)on];
        });
    });
}
...
- (void)clearAllOverrides {
    ...
    [a addAction:[UIAlertAction actionWithTitle:@"Limpar" style:UIAlertActionStyleDestructive handler:^(id _){
        ...
        for (WAGRMethodEntry *e in self.allEntries) {
            ...
            [ud removeObjectForKey:e.name]; // remove legacy raw Runtime Browser key from old builds
            e.overrideState = nil;
        }
        [ud synchronize];
...
- (void)applyFilter:(NSString *)q {
    ...
    [self.tableView reloadData];
    NSUInteger on = 0;
    for (WAGRMethodEntry *e in _filtered) if (WAGRBREffective(e)) on++;
...
- (UITableViewCell *)tableView:(UITableView *)tv cellForRowAtIndexPath:(NSIndexPath *)ip {
    ...
    BOOL effective = WAGRBREffective(e);
    c.textLabel.textColor = effective ? BRGreen() :
                            [e.overrideState isEqualToString:@"off"] ? BROrange() :
                            UIColor.labelColor;

    NSString *sys = e.systemValueKnown ? (e.systemValue ? @"ON" : @"OFF") : @"unknown";
    NSString *ovr = e.overrideState ?: @"system";
    c.detailTextLabel.text = [NSString stringWithFormat:@"effective=%@ · system=%@ · override=%@", effective ? @"ON" : @"OFF", sys, ovr];
...
- (void)segChanged:(UISegmentedControl *)seg {
    ...
    if (seg.selectedSegmentIndex == 2) {
        if (e.systemValueKnown && e.systemValue) {
            e.overrideState = nil;
            [ud removeObjectForKey:WAGRKey(e.name)];
        } else {
            e.overrideState = @"on";
    ...
    [ud removeObjectForKey:e.name]; // legacy raw direct key cleanup
    [ud synchronize];
    NSIndexPath *ip = [NSIndexPath indexPathForRow:seg.tag inSection:0];
    [self.tableView reloadRowsAtIndexPaths:@[ip] withRowAnimation:UITableViewRowAnimationNone];
    NSUInteger on = 0;
    for (WAGRMethodEntry *entry in _filtered) if (WAGRBREffective(entry)) on++;
    self.title = [NSString stringWithFormat:@"%@ (%lu ✓)", _browserTitle, (unsigned long)on];
}
@end
