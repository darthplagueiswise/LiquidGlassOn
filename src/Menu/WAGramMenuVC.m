...
- (NSInteger)tableView:(UITableView*)tv numberOfRowsInSection:(NSInteger)s{
    if (s==0) return 5;   // Masters (switches)
...
- (UITableViewCell*)tableView:(UITableView*)tv cellForRowAtIndexPath:(NSIndexPath*)ip {
    if (ip.section==0) {
        // Master switches
        NSString *titles[] = {@"Liquid Glass", @"Employee / Dogfood", @"Native Debug Menu", @"WAAB Observer", @"Simulate Debug Build"};
        NSString *subs[]   = {@"34 flags: WDSLiquidGlass + WAABProperties", @"isMetaEmployee · isInternalUser · graphQLEmpC1", @"isDebugMenuAllowed = YES", @"Log all WAABProperties calls", @"isDebugBuild = YES → AB Props no Developer menu"};
        NSString *keys[]   = {WA_PREF_LIQUID_GLASS, kWAGREmployeeMaster, kWAGRDebugMenuNative, WA_PREF_AB_OBSERVER, @"wagr_simulate_debug_build"};
        UITableViewCell *c=[[UITableViewCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:nil];
        c.backgroundColor=CELLBG();
...
- (void)masterChanged:(UISwitch*)sw {
    ...
    switch (sw.tag) {
        ...
        case 4: WAGRDebugBuildEnsureHooksInstalled(); break;
    }
    ...
    if (ip.section==3) {
        if (ip.row==0) {
            ...
            UIAlertController *confirm=[UIAlertController
                alertControllerWithTitle:@"Reset completo?"
                message:@"Remove TODOS os overrides wagr.* E chaves nativas espelhadas.\n\nO app será reiniciado após 1.5s para garantir flush no disco."
                preferredStyle:UIAlertControllerStyleAlert];
            [confirm addAction:[UIAlertAction actionWithTitle:@"Resetar e Reiniciar"
                ...
                handler:^(id _){
                    ...
                    // They are raw flag names like "ios_liquid_glass_enabled" = YES.
                    for (NSString *k in allKeys) {
                        // Native LG mirrors
                    ...
                    // ── Step 3: call native LG defaults wipe
                    WAGRLGPrefsDidChange();

                    // ── Step 4: CFPreferencesAppSynchronize — forces real disk write
                    // [ud synchronize] is a no-op on iOS 12+; this is the real flush.
                    [ud synchronize];
                    CFPreferencesAppSynchronize(kCFPreferencesCurrentApplication);

                    // ── Step 5: wait 1.5s for disk write to complete, then exit
