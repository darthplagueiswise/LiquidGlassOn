...
//             master switches usam override mínimo: absent = sistema, true/false só quando diverge.
// Toggles visuais refletem o valor efetivo do sistema sempre que dá para ler com segurança.
...
// ── Pref helpers ──────────────────────────────────────────────────────────────
// Visual state is default-aware:
//   stored "on"  => effective ON
//   stored "off" => effective OFF
//   absent       => real system/default value, when it can be read safely
static BOOL FlagOff(NSString *f) { return WAGRIsOff(f); }

static BOOL FlagSystemDefault(NSString *f, BOOL *knownOut) {
    BOOL known = NO;
    BOOL value = WAGRWAABOriginalBoolForFlag(f, &known);
    if (knownOut) *knownOut = known;
    return known ? value : NO;
}
static BOOL FlagEffective(NSString *f) {
    NSString *stored = WAGRStoredStateForFlag(f);
    if ([stored isEqualToString:@"on"]) return YES;
    if ([stored isEqualToString:@"off"]) return NO;
    BOOL known = NO;
    BOOL sys = FlagSystemDefault(f, &known);
    return known ? sys : NO;
}
static NSString *FlagDetail(NSString *f) {
    NSString *stored = WAGRStoredStateForFlag(f);
    BOOL known = NO;
    BOOL sys = FlagSystemDefault(f, &known);
    BOOL eff = FlagEffective(f);
    if ([stored isEqualToString:@"on"]) return known && sys ? @"✓ ON override (igual ao sistema)" : @"✓ ON override";
    if ([stored isEqualToString:@"off"]) return known && sys ? @"✕ OFF override" : @"✕ OFF override";
    if (known) return eff ? @"✓ ON sistema" : @"sistema OFF";
    return @"sistema";
}
static void FlagSet(NSString *f, BOOL desired) {
    BOOL known = NO;
    BOOL sys = FlagSystemDefault(f, &known);
    if (desired) {
        if (known && sys) WAGRSet(f, nil);
        else WAGRSet(f, @"on");
    } else {
        if (known && sys) WAGRSet(f, @"off");
        else WAGRSet(f, nil);
    }
...
static void FlagReset(NSString *f) { WAGRSet(f, nil); }
...
static BOOL MasterSystemDefault(NSString *key, BOOL *knownOut) {
    if ([key isEqualToString:WA_PREF_LIQUID_GLASS]) {
...
static BOOL MasterEffective(NSString *key) {
    BOOL v = NO;
    if (WAGRBoolOverrideForKey(key, &v)) return v;
    BOOL known = NO;
    BOOL sys = MasterSystemDefault(key, &known);
    return known ? sys : [[NSUserDefaults standardUserDefaults] boolForKey:key];
}
static NSString *MasterDetailSuffix(NSString *key) {
    BOOL explicitValue = NO;
    if (WAGRBoolOverrideForKey(key, &explicitValue)) return explicitValue ? @"override ON" : @"override OFF";
    BOOL known = NO;
    BOOL sys = MasterSystemDefault(key, &known);
    if (known) return sys ? @"sistema ON" : @"sistema OFF";
    return @"sistema";
}
static void MasterSet(NSString *key, BOOL desired) {
    NSUserDefaults *ud = NSUserDefaults.standardUserDefaults;
    BOOL known = NO;
    BOOL sys = MasterSystemDefault(key, &known);
    if (desired) {
        if (known && sys) [ud removeObjectForKey:key];
        else [ud setBool:YES forKey:key];
    } else {
        if (known && sys) [ud setBool:NO forKey:key];
        else [ud removeObjectForKey:key];
    }
    [ud synchronize];
}

static BOOL WAGRValueLooksBoolOverride(id value) {
    if ([value isKindOfClass:NSNumber.class]) return YES;
    if ([value isKindOfClass:NSString.class]) return WAGRStateStringFromObject(value) != nil;
    return NO;
}
static BOOL WAGRShouldRemoveResetKey(NSString *k, id value) {
    if (!k.length) return NO;
    if ([k hasPrefix:@"wagr."] || [k hasPrefix:@"wagr_"]) return YES;
    if ([k hasPrefix:@"wa_lg_"] || [k hasPrefix:@"wa_liquid_glass_"] || [k hasPrefix:@"liquid_glass_"] || [k hasPrefix:@"WALiquidGlass"]) return YES;
    if ([k hasPrefix:@"ios_liquid_glass_"]) return YES;
    if ([k hasSuffix:@".mode"]) return YES;
    if (!WAGRValueLooksBoolOverride(value)) return NO;
    NSArray<NSString *> *prefixes = @[@"aura_", @"ai_", @"ios_", @"status_", @"smb_", @"smbi_", @"wb_", @"mex_", @"waffle_", @"wds_", @"pre_chat_", @"context_menu_", @"newsletter_", @"bug_reporting_", @"enable_reasoning_", @"gen36__"];
    for (NSString *p in prefixes) if ([k hasPrefix:p]) return YES;
    NSArray<NSString *> *fragments = @[@"_enabled", @"_gate", @"killswitch", @"kill_switch", @"liquid_glass", @"benefit", @"dogfood", @"employee", @"internal", @"debug_menu"];
    for (NSString *f in fragments) if ([k containsString:f]) return YES;
    return NO;
}
static NSUInteger WAGRResetWAGramDefaults(void) {
    NSUserDefaults *ud = NSUserDefaults.standardUserDefaults;
    NSDictionary *all = [ud dictionaryRepresentation];
    NSUInteger removed = 0;
    for (NSString *k in all.allKeys) {
        if (WAGRShouldRemoveResetKey(k, all[k])) { [ud removeObjectForKey:k]; removed++; }
    }
    [ud synchronize];
...
- (void)updateTitle {
    NSUInteger on = 0; for (NSString *f in _allFlags) if (FlagEffective(f)) on++;
...
- (UITableViewCell*)tableView:(UITableView*)tv cellForRowAtIndexPath:(NSIndexPath*)ip {
    ...
    BOOL off = FlagOff(flag);
    c.textLabel.textColor = on ? WAGR_GREEN() : (off ? WAGR_RED() : WAGR_LABEL());
    c.detailTextLabel.text = FlagDetail(flag);
    c.detailTextLabel.textColor = on ? WAGR_GREEN() : (off ? WAGR_RED() : WAGR_SEC());
...
- (NSUInteger)onCount { NSUInteger n=0; for(NSString*f in _flags)if(FlagEffective(f))n++; return n; }
...
- (NSString*)tableView:(UITableView*)tv titleForFooterInSection:(NSInteger)s {
    return s==0 ? [NSString stringWithFormat:@"%@\n\nVisual default-aware: sem override reflete o valor real do sistema. ON só persiste quando precisa forçar true; OFF só persiste quando precisa derrubar um default true.", _desc?:@""] : nil;
}
...
- (void)masterTog:(UISwitch*)sw {
    NSUserDefaults *ud = NSUserDefaults.standardUserDefaults;
    for (NSString *f in _flags)  sw.isOn ? FlagSet(f, YES) : FlagSet(f, NO);
    for (NSString *f in _kills)  sw.isOn ? FlagSet(f, NO) : FlagReset(f);
...
- (UITableViewCell*)tableView:(UITableView*)tv cellForRowAtIndexPath:(NSIndexPath*)ip {
    ...
    BOOL on = isCtx ? CtxOn(flag) : FlagEffective(flag);
    UITableViewCell *c = WAGRIconCell(row[@"icon"],row[@"color"],row[@"name"],
        [NSString stringWithFormat:@"%@ • %@", isCtx ? (on?@"✓ ATIVO":@"system") : FlagDetail(flag), row[@"desc"]],
        UITableViewCellAccessoryNone);
    c.selectionStyle = UITableViewCellSelectionStyleNone;
    c.detailTextLabel.textColor = on ? WAGR_GREEN() : WAGR_SEC();
    UISwitch *sw = [[UISwitch alloc] init]; sw.on=on; sw.onTintColor=WAGR_ACCENT(); sw.tag=ip.row;
...
- (CGFloat)tableView:(UITableView*)tv heightForHeaderInSection:(NSInteger)s { return 36; }
- (NSString*)tableView:(UITableView*)tv titleForFooterInSection:(NSInteger)s {
    if(s==0)return@"Masters e bundles são default-aware: absent = sistema; true só força ON; false só força OFF quando o default real está ON.";
    if(s==1)return@"Bundles usam persistência mínima: ligar grava ON só onde o sistema ainda é OFF; desligar grava OFF só onde o sistema é ON.";
...
// master switches do not need associated-object storage.
- (UITableViewCell*)masterCellForRow:(NSInteger)row {
    ...
    NSString *detail = [NSString stringWithFormat:@"%@ • %@", m[row].detail, MasterDetailSuffix(key)];
    UITableViewCell *c=WAGRIconCell(m[row].icon,m[row].clr,m[row].title,detail,UITableViewCellAccessoryNone);
    c.selectionStyle=UITableViewCellSelectionStyleNone;
    BOOL effective = MasterEffective(key);
    c.detailTextLabel.textColor = effective ? WAGR_GREEN() : WAGR_SEC();
    UISwitch *sw=[[UISwitch alloc]init]; sw.on=effective; sw.onTintColor=WAGR_ACCENT(); sw.tag=row;
    [sw addTarget:self action:@selector(masterTog:) forControlEvents:UIControlEventValueChanged];
...
- (UITableViewCell*)bundleCellForRow:(NSInteger)row {
    ...
    if([vc isKindOfClass:WAGRABFlagBrowserVC.class]){WAGRABFlagBrowserVC*b=(WAGRABFlagBrowserVC*)vc;for(NSString*f in b.allFlags)if(FlagEffective(f))on++;tot=b.allFlags.count;}
...
- (void)tableView:(UITableView*)tv didSelectRowAtIndexPath:(NSIndexPath*)ip {
    ...
    if(ip.section==5) {
        if(ip.row==0) {
            ...
            UIAlertController *a=[UIAlertController alertControllerWithTitle:@"Reset completo?" message:@"Remove overrides WAGram, chaves antigas diretas do Runtime Browser e resíduos LiquidGlass/Aura. Não roda no launch; só aqui. Reinicia após 1.5s." preferredStyle:UIAlertControllerStyleAlert];
            [a addAction:[UIAlertAction actionWithTitle:@"Resetar" style:UIAlertActionStyleDestructive handler:^(id _){
                NSUInteger removed = WAGRResetWAGramDefaults();
                NSLog(@"[WAGram][Reset] removed %lu NSUserDefaults keys", (unsigned long)removed);
