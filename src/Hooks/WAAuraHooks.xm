...
// ── Swift WAAuraGating module hooks ──────────────────────────────────────────
// GatedBenefitProvider and GatedSubscriptionProvider are Swift classes in the
// WAAuraGating framework. They gate feature access beyond WAAB AB flags.
// Hook their BOOL zero-arg methods via MSHookMessageEx on the mangled class name.

static BOOL (*orig_gatedBenefitBool)(id,SEL) = NULL;
static BOOL (*orig_gatedSubsBool)(id,SEL) = NULL;

static BOOL hook_gatedBenefitBool(id self, SEL _cmd) {
    NSString *sel = NSStringFromSelector(_cmd);
    if (WAGRIsOn(@"aura_enabled") || WAGRIsOn(@"aura_app_themes_benefit_active")) {
        if ([sel containsString:@"kill"] || [sel containsString:@"block"]) return NO;
        return YES;
    }
    return orig_gatedBenefitBool ? orig_gatedBenefitBool(self,_cmd) : NO;
}
static BOOL hook_gatedSubsBool(id self, SEL _cmd) {
    NSString *sel = NSStringFromSelector(_cmd);
    if (WAGRIsOn(@"aura_enabled") || WAGRIsOn(@"isEligibleForSubscriptions")) {
        if ([sel containsString:@"kill"] || [sel containsString:@"block"]) return NO;
        return YES;
    }
    return orig_gatedSubsBool ? orig_gatedSubsBool(self,_cmd) : NO;
}

static void WAGRHookGatedProvider(NSString *clsName, BOOL (*hook)(id,SEL), BOOL (**origOut)(id,SEL)) {
    if (*origOut) return;
    Class cls = NSClassFromString(clsName);
    if (!cls) return;
    unsigned int n=0; Method *ms = class_copyMethodList(cls,&n); if(!ms) return;
    for (unsigned int i=0;i<n;i++) {
        ...
        IMP orig=NULL;
        MSHookMessageEx(cls,method_getName(ms[i]),(IMP)hook,&orig);
        if (orig && !*origOut) *origOut = (BOOL(*)(id,SEL))orig;
    }
    free(ms);
    NSLog(@"[WAGram][AuraGating] hooked %@", clsName);
}

// Call after WAGRAuraEnsureHooksInstalled
extern "C" void WAGRAuraGatingSwiftHooksInstall(void) {
    WAGRHookGatedProvider(@"_TtC12WAAuraGating20GatedBenefitProvider",
                          hook_gatedBenefitBool, &orig_gatedBenefitBool);
    WAGRHookGatedProvider(@"_TtC12WAAuraGating25GatedSubscriptionProvider",
                          hook_gatedSubsBool, &orig_gatedSubsBool);
}

__attribute__((constructor))
static void WAGRAuraGatingCtor(void) {
    @autoreleasepool {
        double d[]={1.5,3.0,6.0};
        for(int i=0;i<3;i++)
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(d[i]*NSEC_PER_SEC)),
                dispatch_get_main_queue(),^{ WAGRAuraGatingSwiftHooksInstall(); });
    }
}
