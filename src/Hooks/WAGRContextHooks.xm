...
static BOOL WAGRContextCoreHooksReady(void) {
    return origIsDebugBuild && origIsDebugMenuAllowed;
}

extern "C" void WAGRContextEnsureHooksInstalled(void) {
    if (gContextHooksInstalled && WAGRContextCoreHooksReady()) return;
    ...
    free(all);
    gContextHooksInstalled = (gContextHooked > 0);
    NSLog(@"[WAGram][Context] hooks installed=%@, count=%lu core=%@/%@",
          gContextHooksInstalled ? @"YES" : @"NO",
...
static BOOL WAGRContextHasStartupReason(void) {
    NSArray<NSString *> *keys = @[
        kWAGRContextSimulateDebug, kWAGRContextDebugMenu, @"wagr.context.testFlight", @"wagr.context.betaVerbose"
    ];
    for (NSString *key in keys) {
        id obj = WAGRPersistentObjectForKey(key);
        if ([obj respondsToSelector:@selector(boolValue)] && [obj boolValue]) return YES;
    }
...
static void WAGRContextCtor(void) {
    @autoreleasepool {
        if (!WAGRContextHasStartupReason()) {
            NSLog(@"[WAGram][Context] startup inert; no context override active");
            return;
        }
